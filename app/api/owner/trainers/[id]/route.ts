...
function workDuration(checkIn: string, checkOut: string): string {
...
function workMinutes(checkIn: string, checkOut: string): number {
  const ms = Math.max(0, new Date(checkOut).getTime() - new Date(checkIn).getTime())
...
export async function GET(
  ...
  try {
    ...
    const daysElapsed = now.getDate() // working days elapsed this month
    ...
    const [userRes, detailsRes, attendanceRes, salaryRes] = await Promise.all([
      ...
      admin.from("trainer_salaries")
        .select("id,base_salary,bonus,status,paid_at,month_start")
        .eq("user_id", id)
    ...
    // ── Per-day attendance map ───────────────────────────────────────────
    const attendanceByDate = new Map(attendance.map(a => [a.date, a]))

    // ── Calendar: every day elapsed this month ───────────────────────────
    ...
    for (let d = 1; d <= daysElapsed; d++) {
      ...
      const dur = checkOut ? workDuration(rec.check_in_time, checkOut) : null

      if (late) lateDays++
      if (isHalfDay) halfDays++
      if (checkOut) totalWorkMinutes += workMinutes(rec.check_in_time, checkOut)

      const inMins = zonedMinutes(rec.check_in_time)
      checkInMinutesSum += inMins
      checkInCount++
      if (checkOut) {
        checkOutMinutesSum += zonedMinutes(checkOut)
        checkOutCount++
      }
    ...
    const totalHoursWorked = totalWorkMinutes > 0
      ? `${Math.floor(totalWorkMinutes / 60)}h ${totalWorkMinutes % 60}m`
      : null

    // ── Consecutive absence streak from today backwards ──────────────────
    ...
    // ── Today ─────────────────────────────────────────────────────────────
    const todayRecord = attendanceByDate.get(today)
    const checkedInToday = !!todayRecord?.check_in_time
    ...
    if (consecutiveAbsences >= 3) insights.push(`${consecutiveAbsences} consecutive absences detected`)
    ...
    // ── Salary deduction suggestion ───────────────────────────────────────
    const currentSalaryRec = (salaryRes.data ?? []).find((s: any) => s.month_start?.startsWith(monthStart.slice(0, 7)))
    const baseSalary = Number(currentSalaryRec?.base_salary ?? 0)
    const perDayRate = daysElapsed > 0 && baseSalary > 0 ? Math.round(baseSalary / 30) : 0
    const suggestedDeduction = perDayRate * absentDays
    ...
    return ok({
      ...
      attendance: {
        ...
        payrollSuggestion: {
          ...
          perDayRate,
          suggestedDeduction,
          finalSalary: Math.max(0, baseSalary - suggestedDeduction),
        },
        insights,
      },
      clients,
