...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      throw new BadRequestError("Screenshot file is required")
    }
    ...
    if (file.size > MAX_IMAGE_BYTES) {
      throw new BadRequestError("Payment screenshot must be 5MB or smaller")
    }

    const serviceClient = createSupabaseServiceRoleClient()
    const filePath = `payments/${auth.authUserId}/${Math.random()}-${Date.now()}.${extension}`
