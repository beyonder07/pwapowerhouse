...
-- Cloud-first Next.js monolith using Supabase
-- Credentials live in auth.users; public.users stores role/profile metadata.
...
create unique index if not exists idx_users_single_owner on public.users ((role)) where role = 'owner';
...
create index if not exists idx_members_status_expiry_date on public.members(status, expiry_date);
create index if not exists idx_members_trainer_status on public.members(trainer_id, status);
...
create index if not exists idx_payments_member_status_date on public.payments(member_id, status, date desc);
create index if not exists idx_payments_status_date_mode on public.payments(status, date desc, mode);
...
create index if not exists idx_requests_created_by_status_created_at on public.requests(created_by, status, created_at desc);
...
create index if not exists idx_attendance_audits_changed_at on public.attendance_audits(changed_at desc);
create index if not exists idx_activity_audits_actor_user_id on public.activity_audits(actor_user_id);
create index if not exists idx_activity_audits_entity on public.activity_audits(entity_type, entity_id);
create index if not exists idx_activity_audits_created_at on public.activity_audits(created_at desc);
...
as $$
  select exists (
    ...
    where id = target_member_id
      and trainer_id = (select public.current_trainer_id())
  );
$$;
...
alter table public.activity_audits enable row level security;
...
drop policy if exists activity_audits_owner_all on public.activity_audits;
create policy activity_audits_owner_all on public.activity_audits
  for all
  to authenticated
  using (public.current_app_role() = 'owner')
  with check (public.current_app_role() = 'owner');
...
-- Non-owner writes should go through validated Next.js route handlers using the service role key.
-- RLS remains the safety net for any direct database access from authenticated clients.
